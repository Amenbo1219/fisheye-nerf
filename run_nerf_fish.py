...
def train():
    ...
    _, _, focal = hwf
    H, W, focal = hwf
    H, W,focal = images.shape[1],images.shape[2],1.0
    H, W = int(H), int(W)
    hwf = [H, W]
    print("hwf",hwf)
    import cv2
    ...
    if K is None and f_eq is None:
       f_eq = 302
       K = np.array([
            [f_eq, 0, 0.5 * W],  # x軸方向のスケール
            [0, f_eq, 0.5 * H],  # y軸方向のスケール
            [0, 0, 1]
        ])
       print("K",K)
...
if __name__=='__main__':
    ...
    train()
>>>>>>> 28b854e28bee4dfe6c5885b8b8f385c3d78afb82
